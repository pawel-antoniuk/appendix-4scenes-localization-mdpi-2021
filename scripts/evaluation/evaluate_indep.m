...
for iEvaluationIteration = 1:params.EvaluationIterations
    for iHRTFGroup = 1:length(uniqueHRTFGroupNames)    
        ...
        devDs = devDs.filterDs(@(ii) ~strcmp(meta.HRTFGroups(ii), uniqueHRTFGroupNames{iHRTFGroup}));
    
        testDs = ds.filterDs(@(ii) any(strcmp(meta.SongNames(ii), testRecordings))); 
        testDs = testDs.filterDs(@(ii) strcmp(meta.HRTFGroups(ii), uniqueHRTFGroupNames{iHRTFGroup}));
        
        fprintf("HRTF Group: %s\n", uniqueHRTFGroupNames{iHRTFGroup});
        fprintf("all \t\t\t%d\n├─dev \t\t\t%d (%.2f%%)\n└─test \t\t%d (%.2f%%)\n", ...
        ...
        evaluationResults(iEvaluationIteration,iHRTFGroup).Net = net;
        evaluationResults(iEvaluationIteration,iHRTFGroup).Info = info;
        evaluationResults(iEvaluationIteration,iHRTFGroup).LearnRate = params.LearnRate;
        evaluationResults(iEvaluationIteration,iHRTFGroup).MiniBatchSize = params.MiniBatchSize;
        evaluationResults(iEvaluationIteration,iHRTFGroup).MaxEpochs = params.MaxEpochs;
        evaluationResults(iEvaluationIteration,iHRTFGroup).TestAccuracy = testAccuracy;
        evaluationResults(iEvaluationIteration,iHRTFGroup).HRTFGroup = uniqueHRTFGroupNames{iHRTFGroup};
    end
end
